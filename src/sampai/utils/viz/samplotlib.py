"""
Matplotlib-based visualization for Sampai AMR fields.

This module provides tools for visualizing adaptive mesh refinement (AMR)
data from Sampai using matplotlib. It handles the multiresolution nature
...
Example:
    ...
    >>> plotter = viz.FieldPlotter(u, ax=ax)
    >>> for i in range(100):
    >>>     # ... update simulation ...
...
def _extract_cell_data(field) -> Tuple[List, List, List, List]:
    """Extract cell geometry and data from a Sampai field.

    Args:
        field: Sampai ScalarField or VectorField
...
def _extract_vector_data(vector_field) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, List]:
    ...
    Args:
        vector_field: Sampai VectorField with 2 or 3 components
...
def plot_field(
    ...
    **kwargs
    ...
    Args:
        field: Sampai ScalarField to plot
    ...
    Example:
        >>> fig, ax = plt.subplots()
        >>> from sampai.utils import viz
        >>> viz.plot_field(u, ax=ax, cmap='RdBu_r', show_mesh=True)
        >>> plt.show()
    """
    if ax is None:
        fig, ax = plt.subplots()
...
def plot_vector(
    ...
    **kwargs
    ...
    Args:
        vector_field: Sampai VectorField with 2 components
        ax: Matplotlib axes (creates new figure if None)
    ...
    Example:
        >>> fig, ax = plt.subplots()
        >>> from sampai.utils import viz
        >>> viz.plot_vector(velocity, ax=ax, scale=20)
        >>> plt.show()
    """
    if ax is None:
        fig, ax = plt.subplots()
...
def plot_mesh(
    ...
    **kwargs
    ...
    Args:
        field: Sampai field (mesh is extracted from field.mesh)
        ax: Matplotlib axes (creates new figure if None)
    ...
    Example:
        >>> fig, ax = plt.subplots()
        >>> from sampai.utils import viz
        >>> viz.plot_mesh(u, ax=ax, by_level=True)
        >>> plt.show()
    """
    if ax is None:
        fig, ax = plt.subplots()
...
class FieldPlotter:
    ...
    Args:
        field: Initial Sampai ScalarField to plot
    ...
    Example:
        >>> from sampai.utils import viz
        >>> plotter = viz.FieldPlotter(u, cmap='RdBu_r')
        >>> for i in range(100):
        >>>     # ... update simulation ...
        >>>     plotter.update(u)
        >>>     plt.pause(0.01)
    ...
    def update(self, field, title: Optional[str] = None):
        ...
        Args:
            field: New Sampai ScalarField to plot
...
class VectorPlotter:
    ...
    Args:
        vector_field: Initial Sampai VectorField to plot
        ax: Matplotlib axes (creates new if None)
    ...
    Example:
        >>> from sampai.utils import viz
        >>> plotter = viz.VectorPlotter(velocity)
        >>> for i in range(100):
        >>>     # ... update simulation ...
    ...
    def update(self, vector_field, title: Optional[str] = None):
        ...
        Args:
            vector_field: New Sampai VectorField to plot
            title: Optional new title
        """
        # Extract new data
